...
final class FileInput
{
     ...
     * is rejected at upload time unless `$bufferNonSeekable` is true, which
     * reads it NOW into a seekable php://temp copy this input owns (KS04SnqR).
     * The caller owns the stream it passed (the SDK never closes it).
     *
    ...
    public static function resource(
        ...
        if ($bufferNonSeekable) {
            // KS04SnqR: read the pipe NOW into a seekable php://temp copy the
            // input owns; PHP deletes any spill file when the copy is freed.
            $resource = UploadSource::bufferNonSeekable($resource);
        }
